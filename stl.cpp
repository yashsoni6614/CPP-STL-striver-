...
void explainSet()
{
    // set is the data structure in which the things that we are gonna store is gonna be stored in the sorted order and are going to be unique
    set<int> st;
    st.insert(3);
    st.emplace(5);
    st.insert(3);
    ...
    st.insert(8);
    st.insert(1);
    // the finally sorted order will be like this (1,2,3,5,8) no repeated occurence of the values will be there

    auto it = st.find(3); // this find method is gonna return an iterator to that location and if it is not present then it is gonna point to the null

    st.erase(5); // this gonna delete the 5 from the set this takes O(ln) time complexity

    int cnt = st.count(1); // this gonna tell if it is present in the list or not if yes then it gonna write 1 if no then it will be 0

    auto it = st.find(4);
    st.erase(it); // when we are gonna pass the iterator then it is gonna take the constant time for the removal of this thing

    // if we wanna erase from some value to some value in the set then we have to pass the iterator like this
    //  (1,2,3,4,5)
    auto it1 = st.find(2);
    auto it2 = st.find(4);
    st.erase(it1, it2); //(1,4,5);

    // in order to get the first element in here
    auto it3 = st.begin(); //
    cout << *(it3);
}
...
void explainUset()
{
    // unoredred set is same as the ordered set the difference in here is here the element is not stored in the sorted order
}
...
void explainMap()
{
    ...
    // the methods to insert some element inside the map
    m1[1] = 2;          // here 1 is the key and 2 is the value
    m2.insert({4,{2,3}});
    m1.emplace(2,5);
    //map stores the element inside in the sorted key order 
    //this is how we are gonna iterate through the map 
    for (auto &it :m1) {
        cout<<it.first<<","<<it.second;
    }


    auto k = m1.find(3);//this is gonna return the iterator if not found it is gonna return the m1.end()
    cout<< k->second;

}

void explainUnorderedMap() {
    //this is same as the map the difference is the values are not stored in here in the sorted key order 
    //here the searching and the inserting most of the time takes O(1) while in the ordered map it takes O(log n)
...
void explainExtra () {
    //in this one we are gonna see some of the algo which is necessary forr us 
    //this is how we can sort the stuff directly using the standard stl 
    vector<int>v {1,5,7,3,2};
    sort(v.begin(),v.end());
    
    pair<int,int>arr[] = {{1,2},{2,1},{4,1}};
    //here what we want in here is we wanna sort this in this way 
    //sort according to the second element 
    //if second element is same then sort 
    //it according to the first element but in descending

    //comp is a funtion which returns the bool which is set according to the way we want our container to be sorted
    sort(arr,arr+3,comp);

    int num = 7 ; //binary is 00000111
    int cnt = __builtin_popcount(num);//this returns the number of set bits(1) in the binary representation of the number
    //cnt = 3
    ...
    //when all the permutations ends then it returns null
    int ar[] = {1,3,4,5,9};
    int maxi = *(max_element(ar,ar+5));//this returns the iterator 
}
